...
function DateSelector({ settings, bookedDate, cabin }) {
  ...
  // Ensure settings are provided, otherwise use default values
  ...
  return (
    <div className="flex flex-col justify-between">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between px-4 sm:px-8 bg-accent-500 text-primary-800 h-auto sm:h-[72px]">
        <div className="flex flex-col sm:flex-row items-baseline gap-6">
          <p className="flex gap-2 items-baseline">
            {discount > 0 ? (
              ...
              <span className="text-2xl sm:text-lg">${regularPrice}</span>
            )}
            <span className="">/night</span>
          </p>
          {numNights > 0 ? (
            <>
              <p className="bg-accent-600 px-3 py-2 text-2xl sm:text-lg">
        ...
        {range?.from || range?.to ? (
          <button
            className="border border-primary-800 py-2 px-4 mt-4 sm:mt-0 text-sm font-semibold"
            onClick={resetRange}
          >
            Clear
          </button>
